...
const Windy = function (params = {}) {
    ...
    let buildParams = function (params) {
        ...
        that.MAX_VELOCITY_INTENSITY = params.maxVelocity || 60; // velocity at which particle intensity is maximum (m/s)
        that.VELOCITY_SCALE = (params.velocityScale || 0.02) * (Math.pow(window.devicePixelRatio, 1 / 3) || 1); // scale for wind velocity (completely arbitrary--this value looks nice)
        that.MAX_PARTICLE_AGE = params.particleAge || 180; // max number of frames a particle is drawn before regeneration
        that.PARTICLE_LINE_WIDTH = params.lineWidth || 3; // line width of a drawn particle
        that.PARTICLE_MULTIPLIER = params.particleMultiplier || 1 / 1250; // particle count scalar (completely arbitrary--this values looks nice)
    ...
    let animate = function (bounds, field) {
        ...
        for (let i = 0; i < particleCount; i++) {
            particles.push(field.randomize({
                age: Math.floor(Math.random() * that.MAX_PARTICLE_AGE) + 0
            }));
        }
